...
class SeedPoolMinimizer:
    def __init__(self, seed_pool):
        self.seed_pool = seed_pool
    
    def minimize_for_target(self, target_name, output_dir):
        # 从种子池获取语料库
        corpus_dir = self.seed_pool.get_corpus_dir(target_name)
        
        # 获取目标配置
        target_config = self.seed_pool.get_target_config(target_name)
        
        # 创建并运行最小化器
        minimizer = AFLCorpusMinimizer(
            target_bin=target_config['binary'],
            input_dir=corpus_dir,
            output_dir=output_dir,
            threads=target_config.get('threads', 1),
            timeout=target_config.get('timeout', 5000)
        )
        
        # 运行最小化
        selected_files = minimizer.minimize_corpus()
        
        # 更新种子池状态
        self.seed_pool.update_minimized_corpus(
            target_name, 
            output_dir, 
            selected_files
        )
